...
バッチ方針: 承認された項目は行き先スキルごとにグルーピングし、1 スキルにつき 1 回でまとめて委譲する(1 件ずつ起動して往復を増やさない)。具体的には issue-tracker create は複数件を 1 回、issue-tracker done も別途 1 回、update-arch / update-readme もそれぞれ 1 回、record-adr は手動ハンドオフ(下記)。
...
- `### issue-tracker (create)` / `### issue-tracker (done)` / `### update-arch` / `### update-readme` の節に、書き込まれたパス一覧(例: `issues/inbox/2026-07-07-xxx.md`, `docs/arch/flow.md` 等)を箇条書き。
- `### record-adr (手動ハンドオフ待ち)` の節に、タイトル素案・Context / Decision / Rationale 素案・ユーザーに促した `/record-adr "<title>"` コマンドを記載する。実パスは「未確定(手動実行後に確定)」と明記。
...
9. Dispatch 後は完了マニフェストで締める。書き込まれたパス一覧・手動ハンドオフ待ち・保留を提示して出口を明確にする。
