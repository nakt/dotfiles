...
CI 実行中でもユーザーに確認せず、完了まで待つ。
`--watch` はチェックが終わるまで戻らないため、Bash ツールの `timeout` を最大値 `600000` にして以下を実行する。
`--interval 30` は、非 TTY では更新のたびに表が再出力されるため、その出力量を抑える目的で付けている。
...
- CI が存在しない場合（`no checks reported` を出力して exit 1）: スキップして次へ
- 全チェック成功（exit 0）: ステップ 3 へ
- CI 失敗の場合（上記以外の非ゼロ終了）: 失敗内容を報告して終了。修正後に再度 `/pr-merge` を案内
- Bash ツールがタイムアウトした場合: CI は GitHub 側で進行しているので、同じコマンドを再実行して待ち続ける
