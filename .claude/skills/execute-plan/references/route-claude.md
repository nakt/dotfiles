...
見出し不一致 (渡されたタスク番号に一致する `### Task N:` 見出しが見つからない) や着手前の質問など参照先不足が原因の場合は `sonnet` のまま再委譲し、「何ファイル読んでも理解が深まらず、行き詰まっている」が原因の場合は `opus` に昇格させる。

上記の 4 経路のいずれかで implementer を `opus` に昇格させた場合、「何ファイル読んでも理解が深まらず、行き詰まっている」ことが原因の `NEEDS_CONTEXT` で `opus` に昇格させた場合、またはユーザーが実行時に `opus` を明示指定した場合、当該タスクは合意事項 A2 の「昇格済み」となる。
controller は「昇格済み」になった当該タスクについて reviewer も `opus` にする状態を保持し、タスクが完了するまで取り消さない。
1 タスク内で reviewer が複数回起動しても、一度「昇格済み」になれば以降の reviewer は常に `opus` とする。
...
| `NEEDS_CONTEXT` | 見出し不一致 (渡されたタスク番号に一致する `### Task N:` 見出しが見つからない) が原因の場合は `sonnet` のまま fresh subagent を起動し直す。着手前の質問が原因の場合も `sonnet` のまま、不足分の参照先 (プランの見出し名、ファイルパスなど) を「再委譲時の追加指摘」として渡し fresh で再委譲する。ファイルに存在しない情報 (ユーザーからの口頭の指示など) に限って controller が文面を書く。「何ファイル読んでも理解が深まらず、行き詰まっている」が原因の場合は `model=opus` に昇格して再委譲する。原因は controller が報告の文面で見分ける |
