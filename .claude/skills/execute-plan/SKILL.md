...
8. バッチ後チェック: バッチ内全タスクの対象ファイル以外に未コミット変更が残っていないか `git status --porcelain` で確認し、あればスコープ逸脱としてユーザーに報告する
...
- 未完タスクがあれば一覧で報告する。内訳は、エスカレーションでスキップした `deleted` のタスクと、着手前に停止して `pending` のまま残ったタスク
