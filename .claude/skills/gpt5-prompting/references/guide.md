...
3. [Tool Preamble](#3-tool-preambleツール呼び出し前の説明)
4. [reasoning_effort パラメータ](#4-reasoning_effort-パラメータ)
...
18. [まとめ: 実務向けプロンプト構成テンプレート](#まとめ-実務向けプロンプト構成テンプレート)

---

## 1. ロール指定とパーソナリティの区別

GPT-5系リーズニングモデルでは、従来型の**専門性ロール指定は使用しない**。代わりにパーソナリティ定義でトーン・スタイルを制御する。
