...
検出・検証のコマンドと閾値 (6 行) は [tools.md](tools.md) の「dedupe モード」に集約している。本ファイルは統合の判断基準と改善パターンを扱う。
...
差分が 1, 2 個ならクラス化せず高階関数 (`build(load_fn, metrics_fn, kind)`) でも良い。基底クラスを新しいモジュールに置く場合はファイル新規作成になるので、作る前にユーザー確認を取る。
...
| 2 箇所 | 下の追加条件を両方満たすときのみ統合。満たさなければ複製のまま残す |
| 1 箇所 | pylint の重複としては報告されない |

2 箇所の場合の追加条件:

- ブロックが閾値の 6 行以上ある (3 回ルールが守ろうとしているのは「数行の小さなパターンを早すぎる抽象化から守ること」なので、6 行以上の完全重複には適用しない)
- 2 箇所が同じドメイン概念を扱っている
...
コマンドは [tools.md](tools.md) の「検証コマンド集」を参照。閾値 6 行を指定した pylint で判定する。

- [ ] 閾値 6 行の pylint で、統合すると判断した重複が残っていない (据え置くと判断した重複の残存は可。理由を 1 行で示す)
- [ ] 命名・シグネチャが揃っている
