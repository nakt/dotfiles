...
- 20260731 判断を一部変更: 保存先は `docs/workflow/memo/` のままとしつつ、`.gitignore` に追加して git 管理外にした
  - 経緯: 同じ日に確定した「設計メモとプランの合意事項は実装完了までの作業ファイルで、実装後は破棄してよい」という寿命の整理と、上記の「git 管理下に残す」が食い違っていた。寿命の整理を正とし、ファイル自体は削除せずローカルに残す形にした。これでプランと設計メモの扱いが揃い、後から読み返すこともできる。
